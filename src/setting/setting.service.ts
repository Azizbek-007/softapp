...
export class SettingService {
  ...
  async create(createSettingDto: CreateSettingDto, api_path: string): Promise<void> {
    ...
    try {
      const response = await axios.get(
        `https://api.telegram.org/bot${createSettingDto.bot_token}/getMe`,
      );
      await axios.get(
        `https://api.telegram.org/bot${createSettingDto.bot_token}/setWebhook?remove`,
      );
      createSettingDto.path = `https://bots.sales-up.uz/bots/${bot_id}.php`;
      await axios.get(
        `https://api.telegram.org/bot${createSettingDto.bot_token}/setWebhook?url=${createSettingDto.path}`,
      );
      createSettingDto.bot_username =
        'https://t.me/' + response.data.result?.username;
  ...
  async send_message(filename: string, sendMessageDto: SendMessageDto) {
    ...
    if (!sendMessageDto.user_id) {
      const users = await this.LeadRepository.findBy({
        user_id: Not(IsNull()),
      });
